...
class Truss():
    def __init__(self,view,name,db_dir,db_indices,port,area_x,area_y,area_z,interference_area_x,reset_x):
        ...
        self.stacks_file = f'{db_dir}/stacks/{name}.json'
        ...
        self.server.register_area(snap7.types.srvAreaDB, db_indices[1], self.db1)
    ...
    async def moving_x(self,index):
        ...
        for interval in np.linspace(x_start,x_end,int(abs(x_start - x_end) / 200)+1):
    ...
    async def moving_r(self,index):
        ...
        for interval in np.linspace(start,end,int(abs(start - end) / 10)+1):
    ...
    async def update(self):
        while True:
            ...
            if index == 2 and self.db2:
                if action & 0b00010000: #上锁
                    if self.db2[2] & 0b00010000 and self.db3[35] & 0b00000001:
                        self.db1[35] = self.db1[35] & 0b11111110
                    else:
                        self.db1[35] = self.db1[35] | 0b00000001
                else: #解锁
                    self.db1[35] = self.db1[35] & 0b11111110
                    if self.db2[2] & 0b00010000:
                        self.db3[35] = self.db3[35] | 0b00000001
                pass
            elif index <= 1: 
                ...
                if action == 1:
                    ...
                    # print('复位指令')
                    if index == 0:
                        self.db0[38],self.db0[39],self.db0[40],self.db0[41] = pack('!f',self.reset_x[0])
                        ...
                        self.db0[70],self.db0[71],self.db0[72],self.db0[73] = pack('!f',self.reset_x[1])
            ...
            self.stacks = dson(self.stacks_file)
        ...
        try:
            ...
            #画托盘
            for drop_pos,pallet_size in [
                    (self.params['placing.pallet_pos'],self.params['placing.pallet_size']),
                    (self.params['placing.pallets.left.pos'],self.params['placing.pallets.left.size']),
                    (self.params['placing.pallets.middle.pos'],self.params['placing.pallets.middle.size']),
                    (self.params['placing.pallets.right.pos'],self.params['placing.pallets.right.size'])]:
                if not drop_pos: continue
                p0 = drop_pos
            ...
            #画码垛
            for pallet_name,stacks in self.stacks.value.items():
                pos = self.params[f'placing.pallets.{pallet_name}.pos']
                for stack in stacks:
                    for name,offset,part_size,kind,real_size in stack:
                        p0 = pos[0] + offset[0], pos[1] + offset[1], pos[2] + offset[2]
                        # p1 = pos[0] + offset[0], pos[1] + offset[1] + part_size[1], pos[2] + offset[2]
                        # p2 = pos[0] + offset[0] + part_size[0], pos[1] + offset[1] + part_size[1], pos[2] + offset[2]
                        # p3 = pos[0] + offset[0] + part_size[0], pos[1] + offset[1], pos[2] + offset[2]
                        # self.rectangle(p0,p1,p2,p3)
                        center = p0[0] + part_size[0] / 2,p0[1] + part_size[1] / 2,p0[2] # 放置点

                        p0 = center[0] - real_size[0] / 2, center[1] - real_size[1] / 2, center[2]
                        p1 = p0[0],                 p0[1] + real_size[1], center[2]
                        p2 = p0[0] + real_size[0],  p0[1] + real_size[1], center[2]
                        p3 = p0[0] + real_size[0],  p0[1], center[2]
                        self.rectangle(p0,p1,p2,p3)
                        # self.view.text(p3[0],p3[1],p3[2],s=str(name))
            ...
            for part in self.plate['parts']:
                ...
                drop_pos,_,_,pallet_name = drop_positions[id]
                drop_pos_offset = self.params['placing.pallet_pos']
